...
$B\bX\sim{\mathcal{N}_m}\left(\bzero,B^T\Sigma B\right)$ for any constant
matrix~$B\in\mathbb{R}^{n\times m}$, and
analogously $\bZ\sim\cnorm{\bzero}{\Gamma}$
implies~$B\bZ\sim\mathcal{N}\mathcal{C}_m\left(\bzero,\herm{B}\Gamma B\right)$,
$B\in\mathbb{C}^{n\times m}$.  Similar
